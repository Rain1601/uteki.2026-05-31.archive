...
import { REAL_VERDICTS, REAL_NEWS_READS, type FinishedVerdict, type RealNewsRead } from '../../mocks/realData';
...
function VerdictCard({ v }: { v: FinishedVerdict }) {
  const t = useT();
  const { lang } = useI18n();
  ...
  return (
    <div className="rounded border border-ink-faint/30 bg-[#1a1612]/60 p-5 md:p-6">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <Score name="Buffett" v={v.philosophy.buffett} />
        <Score name="Fisher"  v={v.philosophy.fisher} />
        <Score name="Munger"  v={v.philosophy.munger} />
      </div>

      <div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-wider text-ink-faint pt-2">
        <span>{t(STRINGS.companyPage.holdHorizon)}: {lang === 'zh' ? v.hold_horizon_zh : v.hold_horizon_en}</span>
        <a
          href="/company-agent"
